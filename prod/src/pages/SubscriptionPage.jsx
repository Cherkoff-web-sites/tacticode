...
export function SubscriptionPage() {
  ...
  const visibleCount = isMobile ? Math.min(displayedCount, totalSubscriptions) : totalSubscriptions;
  const hasMore = isMobile && displayedCount < totalSubscriptions;
  ...
  useEffect(() => {
    if (isMobile) {
      ...
      setDisplayedCount(totalSubscriptions);
    }
  }, [isMobile, totalSubscriptions]);

  return (
    <>
      <section>
        <div className={containerClass}>
          <h1 className="h2 text-center md:text-left">Оформить подписку</h1>
          ...
          {hasMore && (
            <div className="flex justify-center">
              <button
                type="button"
                onClick={() => setDisplayedCount((prev) => Math.min(prev + 3, totalSubscriptions))}
